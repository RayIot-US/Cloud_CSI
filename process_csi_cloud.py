...
# ========== GitHub Setup ==========
GITHUB_TOKEN = os.getenv("GITHUB_TOKEN")  # Set this in Render or env
GITHUB_USER = "RayIot-US"
GITHUB_REPO = "Cloud_CSI"
INPUT_FILE_PATH = "csi_data/raw_csi_data.txt"
OUTPUT_FILE_PATH = "csi_data/output.csv"
...
def get_file_from_github(filepath):
    print(f"📥 Fetching {filepath} from GitHub")
...
def upload_to_github(content, path):
    url = f"{API_URL}/{path}"
    print(f"📤 Uploading to GitHub: {path}")
    ...
    payload = {
        "message": "Updated processed CSI output (CSV format)",
...
def process_csi(text):
    ...
    while i < len(lines):
        ...
        csi_line = lines[i].strip()
        i += 1
        if not csi_line.startswith("CSI Data:"):
            continue

        csi_values_str = csi_line[len("CSI Data:"):].strip()
        tokens = csi_values_str.split()
        try:
        ...
        imaginary, real = numbers[::2], numbers[1::2]
        amplitudes = [sqrt(i**2 + r**2) for i, r in zip(imaginary, real)]
        phases = [atan2(i, r) for i, r in zip(imaginary, real)]
        ...
        result.append(str(amplitudes))
        result.append(str(phases))
        result.append("")  # spacing between entries
...
if __name__ == "__main__":
    raw_data = get_file_from_github(INPUT_FILE_PATH)
    if raw_data:
        output = process_csi(raw_data)
        upload_to_github(output, OUTPUT_FILE_PATH)
